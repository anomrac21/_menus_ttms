  ...
  async exportMenuToJSON() {
    ...
    const menuData = {
      ...
      categories: this.state.categories.map(cat => ({
        ...
        url: cat.url,
        weight: cat.weight || 0,
        image: cat.image || cat.params?.images?.top || null,
        summary: cat.summary || '',
        images: {
          top: cat.params?.images?.top || cat.image || null,
          bottom: cat.params?.images?.bottom || cat.params?.slidein?.slideinimage || null
        },
        params: {
          ...cat.params,
          images: {
            top: cat.params?.images?.top || cat.image,
            bottom: cat.params?.images?.bottom || cat.params?.slidein?.slideinimage
          }
        }
      })),
  ...
  importMenuFromJSON(jsonString) {
    try {
      ...
      this.state.categories = menuData.categories.map(cat => ({
        ...
        url: cat.url,
        weight: cat.weight || 0,
        image: cat.image || cat.params?.images?.top || cat.params?.image,
        summary: cat.summary || '',
        params: {
          ...cat.params,
          images: {
            top: cat.params?.images?.top || cat.image || cat.params?.image,
            bottom: cat.params?.images?.bottom || cat.params?.slidein?.slideinimage
          }
        }
      }));
      ...
      html += `
        <div class="main-menu-bg ${category.title}">
          <div class="main-menu ${category.title} item">
            <div class="menu-header">
              <a class="menu-anchor" id="${category.title}"></a>
              ${(category.images?.top || category.image) ? `
                <a href="${category.url}">
                  <img class="food item" src="${category.images?.top || category.image}" alt="${category.title}" loading="lazy">
                </a>
              ` : ''}
              <div class="headerstyle item">
                <h2 class="center title">
                  <a href="${category.url}">${category.title}</a>
                  ${(category.images?.bottom || category.params?.slidein?.slideinimage) ? `
                    <img class="slideinimg" 
                         src="${category.images?.bottom || category.params?.slidein?.slideinimage}" 
